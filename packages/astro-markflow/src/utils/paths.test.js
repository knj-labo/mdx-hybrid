...
describe("stripQuery", () => {
  ...
  test("returns input as-is for null or undefined", () => {
...
describe("deriveAstroUrl", () => {
  test("returns undefined for null or empty filePath", () => {
    expect(deriveAstroUrl(null)).toBe(undefined);
  ...
  test("uses process.cwd() when rootDir not provided", () => {
    // This test assumes process.cwd() is the project root
    // Hard to test without mocking, so we'll just verify it doesn't crash
    const filePath = "/some/path/src/pages/test.md";
    const result = deriveAstroUrl(filePath);
    // Result will be undefined unless filePath happens to match cwd structure
    expect(result).toBeOneOf([undefined, expect.any(String)]);
  });

  test("handles paths on current platform", () => {
    // Path separators are normalized based on platform
    // On Unix: use /  On Windows: use \\ (but normalized internally)
