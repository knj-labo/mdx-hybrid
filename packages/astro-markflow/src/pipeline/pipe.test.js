...
describe('pipe', () => {
  ...
  it('should work with object transformations', async () => {
    ...
    const incrementCount = (obj) => ({ ...obj, count: (obj.count || 0) + 1 });
...
describe('when', () => {
  it('should execute transform when condition is true', async () => {
    const transform = when(true, (x) => x * 2);
  ...
  it('should skip transform when condition is false', async () => {
    const transform = when(false, (x) => x * 2);
    const result = await transform(5);
  ...
  it('should evaluate dynamic condition with function', async () => {
    const transform = when((x) => x > 10, (x) => x * 2);
  ...
  it('should work with async transforms', async () => {
    const asyncTransform = when(true, async (x) => {
      return x + 10;
    });
  ...
  it('should work in pipeline composition', async () => {
    const pipeline = pipe(
      (x) => x + 1,
      when(false, (x) => x * 100), // Skipped
      when(true, (x) => x * 2),    // Applied
      (x) => x - 3
    );
...
describe('tap', () => {
  ...
  it('should work in pipeline composition', async () => {
    ...
    const pipeline = pipe(
      (x) => x + 1,
      tap((x) => log.push(`after add: ${x}`)),
      (x) => x * 2,
      tap((x) => log.push(`after mult: ${x}`)),
      (x) => x - 3
    );
  ...
  it('should not affect data flow even if side effect throws', async () => {
    // Note: In current implementation, error would propagate
  ...
  it('should handle object mutations in side effect', async () => {
    const metadata = { callCount: 0 };
    const transform = tap((data) => {
      metadata.callCount++;
...
describe('integration', () => {
  it('should combine pipe, when, and tap', async () => {
    ...
    const pipeline = pipe(
      (x) => x + 1,
      tap((x) => log.push(`step1: ${x}`)),
      when((x) => x % 2 === 0, (x) => x * 10), // Only for even numbers
      tap((x) => log.push(`step2: ${x}`)),
      (x) => x + 5
    );
